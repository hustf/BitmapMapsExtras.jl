...
function principal_curvature_components(z::Matrix, pt::CartesianIndex)
    Ri, Ω, v, P, K, vα, vκ, vβ, _ = _allocations_curvature(CartesianIndices(z), [])
    if ! (pt ∈ Ri)
...
We convert the signed scalar + angle representation into a column vector per component. This is
...
# WAS: principal_curvature_and_direction(vκ, vϕ) = principal_curvature_and_direction(SVector{4, Float64}(vκ), vϕ)

principal_curvature_and_direction(vκ, vϕ) = principal_curvature_and_direction(SVector{4, Float64}(vκ), SVector{4, Float64}(vϕ))

function _allocations_curvature(R::CartesianIndices, directions; maxglyph = 50, minglyph = -50)
...
function func_is_glyph_within_limits(directions, maxglyph, minglyph)
    # Captures variables maxglyph (checked to be > 0) and minglyph (checked to be < 0)
    @assert maxglyph >= 0
    @assert minglyph < maxglyph
    if 1 ∈ directions && 2 ∈ directions
        # Checks a glyph for max and min principal directions. 
        # Input K is a (scaled) 4x4 matrix.
        f_is_within_limits = let maxglyph=maxglyph, minglyph=minglyph, minglyph_abs=abs(minglyph)
            @inline function(K)
                K1 = @view K[:, 1]
                K2 = @view K[:, 2]
                limit1 = is_quantity_positive(K1) ? maxglyph : minglyph_abs
                limit2 = is_quantity_positive(K2) ? maxglyph : minglyph_abs
    ...
    elseif 2 ∈ directions || 1 ∈ directions
        ...
        f_is_within_limits = let maxglyph=maxglyph, minglyph=minglyph, minglyph_abs=abs(minglyph)
            @inline function(v)
                limit = is_quantity_positive(v) ? maxglyph : minglyph_abs
                norm(v) ≤ limit
            end
        end
    else
