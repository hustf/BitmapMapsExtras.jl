...
# This revision replaces gradient! with 𝐧ₚ!, the normalized
# surface normal vector projected onto the base plane.
...
Returns a measure [0 to 1> is never reached, based
on the surface normal unit vector. For testing, this can be done simpler.
"""
function steepness(M)
    1 - tangent_basis(M)[9]
end
...
Returns a measure <-1 to 1> based
on the surface normal unit vector. For testing, this can be done simpler.
"""
function northness(M)
    tangent_basis(M)[8]
end
