...
"""
    interpolate_unit_square(corners::SMatrix{2, 2, TENSORMAP}, xus::AbstractFloat, yus::AbstractFloat)

Implemented here since we couldn't make Interpolations.jl's return @inferred.
"""
function interpolate_unit_square(corners::SMatrix{2, 2, TENSORMAP}, xus::AbstractFloat, yus::AbstractFloat)
    x = clamp(xus, 0.0, 1.0)
    y = clamp(yus, 0.0, 1.0)
    if xus !== x
        throw(" ooo: x")
...
function update_corners!(bid::BidirectionInDomain, i1, j1, i2, j2)
    ...
    if i1 != i2 && j1 != j2
        ...
        mc[1, 1] .= bid.bdog(CartesianIndex{2}((i1, j1)))
        mc[2, 1] .= bid.bdog(CartesianIndex{2}((i2, j1)))
        mc[1, 2] .= bid.bdog(CartesianIndex{2}((i1, j2)))
        mc[2, 2] .= bid.bdog(CartesianIndex{2}((i2, j2)))
    elseif i1 == i2 && j1 != j2
