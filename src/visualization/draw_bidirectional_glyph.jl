...
"""
    plot_principal_directions_glyph!(bbuf, pt, directions, f_is_within_limits, dashsize, K)
...
function draw_two_arrow_glyph!(img, p, v::AbstractVector)
    ...
    if is_bidirec_vect_positive(v)
        ...
        draw_vector!(img, p, -Δi, Δj)
        draw_vector!(img, p, Δi, -Δj)
    else
        # Third or fourth quadrant. Negative.
        draw_vector!(img, p + CartesianIndex(-Δi, Δj), Δi, -Δj)
        draw_vector!(img, p + CartesianIndex(Δi, -Δj), -Δi, Δj)
    end
end
...
function func_is_glyph_within_limits(directions, maxglyph, minglyph)
    ...
    if 1 ∈ directions && 2 ∈ directions
        ...
        f_is_within_limits = let maxglyph=maxglyph, minglyph=minglyph, minglyph_abs=abs(minglyph)
            @inline function(K)
                K1 = @view K[:, 1]
                K2 = @view K[:, 2]
                limit1 = is_bidirec_vect_positive(K1) ? maxglyph : minglyph_abs
                limit2 = is_bidirec_vect_positive(K2) ? maxglyph : minglyph_abs
    ...
    elseif 2 ∈ directions || 1 ∈ directions
        ...
        f_is_within_limits = let maxglyph=maxglyph, minglyph=minglyph, minglyph_abs=abs(minglyph)
            @inline function(v)
                limit = is_bidirec_vect_positive(v) ? maxglyph : minglyph_abs
                norm(v) ≤ limit
            end
        end
    else
